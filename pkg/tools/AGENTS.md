...
- **`grep_search` is time-bounded (`grepSearchTimeout`, 25s).** A recursive search over a huge tree outside the working directory (e.g. `~/go/pkg/mod`, `node_modules`) can otherwise hang the whole agent turn indefinitely (observed in a real session where the agent searched the Go module cache). Both the ripgrep path (`exec.CommandContext`) and the Go fallback (context check in the walk) honor it; on timeout the tool returns an actionable error telling the model to narrow `search_path`/`include_globs`. Do not remove the timeout.
