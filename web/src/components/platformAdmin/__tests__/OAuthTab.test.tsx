...
describe('platform OAuthTab', () => {
  ...
  it('selects both catalog permissions by default and sends only checked values', async () => {
    ...
    expect(screen.getByText('Leave empty to allow both built-in MCP and A2A resources. Enter one or more URLs only to restrict token audiences.')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Create' }))
    expect(adminApi.createOAuthClient).toHaveBeenCalledWith(expect.objectContaining({ resources: [], scopes: ['tool:execute'] }))
  })
})
