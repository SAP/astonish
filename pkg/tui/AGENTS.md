...
6. Capability-gated commands (`/provider`, `/rollback`, `/compact`, `/init`, `/init-deep`) are exposed only when the active backend implements the matching optional interface (`backend.ProviderAdminBackend`, `backend.RollbackBackend`, `backend.CompactionBackend`, `backend.InitBackend`), which only the code-mode `localAgentBackend` does (the lazy `lazyCodeBackend` forwards them once code mode is opened). Gate them in three places — `handleSlash`, `syncSlashCompletion` (the `extra` slash commands), and `helpText` — via `m.providerAdmin()` / `m.rollbackCap()` / `m.compactionCap()` / `m.initCap()`. Never expose these on platform chat. The `/provider` list shows **only** local config.yaml providers (all editable); code mode never fetches or displays platform providers. `/init` (root AGENTS.md) and `/init-deep` (root + per-sub-folder AGENTS.md) differ from `/plan`: they are **not** runtime-gated modes — `runInit` (`init.go`) dispatches an **unrestricted** `RunTurn` with `initSystemContext` / `initDeepSystemContext` injected as `TurnOptions.SystemContext` (no `PlanMode`/`GraphPlanMode`/`AskMode`) so the agent is free to call `write_file`. Those two prompt constants live only on the TUI side (there is no agent-side source of truth to sync, precisely because there is no runtime gate); they steer the agent to explore with `codegraph_explore` first and write the AGENTS.md file(s). `backend.InitBackend.SupportsInit()` reports true only when code mode has a host working directory.
