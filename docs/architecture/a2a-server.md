...
The service retains the supported A2A operations for synchronous and return-immediately message handling, task retrieval, cancellation, push-notification configuration, and streaming. Before each channel-backed A2A execution, the daemon applies the same request-scoped runtime preparation as Studio chat: tenant MCP groups are resolved from the attached scoped stores, and the cascade-selected web-search tool and prompt metadata are injected into the runner. Request-scoped non-MCP tools, including Perplexity, are merged into both semantic `search_tools` results and the complete tool inventory, then resolved by `describe_tools` and `execute_tool`; they are not merely present in an execution-only context. Requests retain the 1 MiB body limit. For official v1 `SendMessage` responses, completed output is projected exactly once as a response artifact; the status remains lifecycle-only and history retains the user input without a duplicate agent-output entry. SSE events are flushed as complete protocol events, with comment-only `: keepalive` frames every 15 seconds while execution is idle so intermediaries do not treat a long tool call as a dead stream. A successful stream emits an initial content-free `working` status, every intermediate user-facing model turn as an ordered `working` progress status, safe tool lifecycle statuses (tool display name and started/completed state only), each result as an `artifactUpdate`, and one content-free `completed` status with `final: true`. Discovery operations retain their own identities (`search_tools` and `describe_tools`), while lifecycle events for the generic `execute_tool` wrapper use the nested tool name requested by that call and correlate the completion by call ID (with ordered fallback when providers omit IDs). Tool arguments, credentials, and tool responses are never projected into lifecycle statuses. Finality is determined at the channel event boundary: text associated with continuing tool execution is progress, while the last completed text turn is the response artifact. Model narration remains an atomic A2A message rather than token-level protocol events. The A2A projection does not compare, buffer, or deduplicate message content, so separately emitted events remain distinct even when their text is equal. The stream does not append a redundant full-task snapshot; internal task persistence remains unchanged for legacy operations and ownership enforcement.
