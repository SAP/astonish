...
6. After the template is known, **do not call ` + "`create_deck`" + ` yet.** Call ` + "`list_slide_templates`" + ` if you do not already have its variant list and ` + "`palettes`" + `. Then:
   ...
   - **Cover photo** — only if the chosen title kind has a ` + "`ph-pic-*`" + ` well. First ` + "`kind: \"yesno\"`" + `: "This cover has room for one photo from the template. Want to pick one?" If **yes**: ` + "`slidesImagePicker: true`" + ` + ` + "`slidesTemplate`" + `, omit options (photos from the template's **example slides**, not only the layout default). Pass the chosen option id (` + "`sha256-…`" + `) as ` + "`titleImage`" + ` on ` + "`create_deck`" + `. If **no**: pass ` + "`titleImage: \"none\"`" + ` — the server leaves the well empty and does **not** keep sample photos. A second photo later is ` + "`add_deck_image`" + `, not another title variant. Skip this pair when the cover has no image well or the template has no example photos.
   ...
   If there is exactly one title or closing, use it — no question. **VIOLATION — never skip the title-variant question when 2+ title* exist, and never skip the cover-photo question when the chosen cover has a ph-pic well.** Still no chrome-picker stall for section/agenda unless the user asked.

### 2. Create the deck

Do **not** call ` + "`create_deck`" + ` until title / cover-photo / palette / closing questions that still apply have been answered. Pass ` + "`template`" + `. Pass a human ` + "`title`" + ` and a short ` + "`description`" + ` (the chat card shows the description, never the persist slug). ` + "`slug`" + ` is a hint only: in chat the server stores a unique per-session slug so two chats on the same topic cannot overwrite each other — use the slug ` + "`create_deck`" + ` returns; if you pass the hint to ` + "`fill_slides`" + `, the server remaps it to this session's deck. Pass ` + "`palette`" + ` (palette id, not hex) when they chose a colorway. Pass ` + "`titleKind`" + ` / ` + "`closingKind`" + ` as the **option id** from ` + "`ask_user`" + ` (the catalog kind: ` + "`title`" + `, ` + "`title-2`" + `, …). "Use the default" → pass ` + "`titleKind: \"default\"`" + `, do not omit. Pass ` + "`titleImage`" + ` as the photo picker's option id, or ` + "`\"none\"`" + ` if they declined. ` + "`create_deck`" + ` errors if those are missing on a template with several covers — that is a skip, not a shortcut. It returns a slim catalog (kind, label, tier, fillSlots, slotHints, summary) plus optional ` + "`palettes`" + ` and ` + "`styleGuide`" + `. It does not return markup.
...
- [ ] Did not ask to generate images; cover photo is slidesImagePicker from template examples or titleImage none
...
- [ ] Slide 0 is the chosen official title kind when listed (not a different variant); last is official closing when listed, else recipe-closer; body is recipe-*; cover photo only if they picked one
