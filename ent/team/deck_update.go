...
func (_u *DeckUpdate) check() error {
	if v, ok := _u.mutation.Slug(); ok {
		if err := deck.SlugValidator(v); err != nil {
			return &ValidationError{Name: "slug", err: fmt.Errorf(`team: validator failed for field "Deck.slug": %w`, err)}
		}
	}
	if v, ok := _u.mutation.Title(); ok {
		if err := deck.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`team: validator failed for field "Deck.title": %w`, err)}
		}
	}
	return nil
}
...
func (_u *DeckUpdateOne) sqlSave(ctx context.Context) (_node *Deck, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`team: missing "Deck.id" for update`)}
	}
