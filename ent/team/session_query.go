...
type SessionQuery struct {
	...
	withChatEvents       *ChatSessionEventQuery
...
// QueryChatEvents chains the current query on the "chat_events" edge.
func (_q *SessionQuery) QueryChatEvents() *ChatSessionEventQuery {
	query := (&ChatSessionEventClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withChatEvents:       _q.withChatEvents.Clone(),
...
// WithChatEvents tells the query-builder to eager-load the nodes that are connected to
// the "chat_events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SessionQuery) WithChatEvents(opts ...func(*ChatSessionEventQuery)) *SessionQuery {
	query := (&ChatSessionEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withChatEvents = query
	return _q
}
...
func (_q *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := _q.withChatEvents; query != nil {
		if err := _q.loadChatEvents(ctx, query, nodes,
			func(n *Session) { n.Edges.ChatEvents = []*ChatSessionEvent{} },
			func(n *Session, e *ChatSessionEvent) { n.Edges.ChatEvents = append(n.Edges.ChatEvents, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SessionQuery) loadChatEvents(ctx context.Context, query *ChatSessionEventQuery, nodes []*Session, init func(*Session), assign func(*Session, *ChatSessionEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Session)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chatsessionevent.FieldChatSessionID)
	}
	query.Where(predicate.ChatSessionEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.ChatEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
