...
func (_c *SessionCreate) SetLastSeq(v int64) *SessionCreate {
	_c.mutation.SetLastSeq(v)
	return _c
}

// SetNillableLastSeq sets the "last_seq" field if the given value is not nil.
func (_c *SessionCreate) SetNillableLastSeq(v *int64) *SessionCreate {
	if v != nil {
		_c.SetLastSeq(*v)
	}
	return _c
...
func (_c *SessionCreate) AddEventIDs(ids ...int64) *SessionCreate {
...
func (_c *SessionCreate) AddChatEventIDs(ids ...uuid.UUID) *SessionCreate {
	_c.mutation.AddChatEventIDs(ids...)
	return _c
}

// AddChatEvents adds the "chat_events" edges to the ChatSessionEvent entity.
func (_c *SessionCreate) AddChatEvents(v ...*ChatSessionEvent) *SessionCreate {
	ids := make([]uuid.UUID, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddChatEventIDs(ids...)
}
...
func (_c *SessionCreate) defaults() {
	...
	if _, ok := _c.mutation.Metadata(); !ok {
		v := session.DefaultMetadata
		_c.mutation.SetMetadata(v)
	}
	if _, ok := _c.mutation.LastSeq(); !ok {
		v := session.DefaultLastSeq
		_c.mutation.SetLastSeq(v)
	}
...
func (_c *SessionCreate) check() error {
	if _, ok := _c.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`team: missing required field "Session.title"`)}
	}
	if _, ok := _c.mutation.MessageCount(); !ok {
		return &ValidationError{Name: "message_count", err: errors.New(`team: missing required field "Session.message_count"`)}
	}
	if _, ok := _c.mutation.FleetKey(); !ok {
		return &ValidationError{Name: "fleet_key", err: errors.New(`team: missing required field "Session.fleet_key"`)}
	}
	if _, ok := _c.mutation.FleetName(); !ok {
		return &ValidationError{Name: "fleet_name", err: errors.New(`team: missing required field "Session.fleet_name"`)}
	}
	if _, ok := _c.mutation.IssueNumber(); !ok {
		return &ValidationError{Name: "issue_number", err: errors.New(`team: missing required field "Session.issue_number"`)}
	}
	if _, ok := _c.mutation.Repo(); !ok {
		return &ValidationError{Name: "repo", err: errors.New(`team: missing required field "Session.repo"`)}
	}
	if _, ok := _c.mutation.LastSeq(); !ok {
		return &ValidationError{Name: "last_seq", err: errors.New(`team: missing required field "Session.last_seq"`)}
	}
	switch _c.driver.Dialect() {
	case dialect.MySQL, dialect.SQLite:
		if _, ok := _c.mutation.CreatedAt(); !ok {
			return &ValidationError{Name: "created_at", err: errors.New(`team: missing required field "Session.created_at"`)}
		}
	}
	switch _c.driver.Dialect() {
	case dialect.MySQL, dialect.SQLite:
		if _, ok := _c.mutation.UpdatedAt(); !ok {
			return &ValidationError{Name: "updated_at", err: errors.New(`team: missing required field "Session.updated_at"`)}
		}
	}
	if v, ok := _c.mutation.ID(); ok {
		if err := session.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`team: validator failed for field "Session.id": %w`, err)}
		}
	}
	return nil
}
...
func (_c *SessionCreate) createSpec() (*Session, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.ChatEventsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
