...
**Browser-in-sandbox.** Direct K8s uses the same browser `Manager` callbacks as the other sandbox backends, but tunnels through `Backend.ExecStreaming` instead of Incus or OpenShell APIs. Browser tools resolve the current chat session, launch CloakBrowser/Chromium inside `/sandbox/rootfs` via `/usr/local/bin/astonish-shell`, keep that exec stream open so the browser stack stays alive, and connect Rod to CDP through a `socat` tunnel on port `9222`. The PDF export browser manager is wired to the same K8s callbacks, using the session ID as the tunnel handle; it does not fall back to Incus or to a non-Chrome renderer. When KasmVNC is installed, the launcher uses `Xkasmvnc :0` and exposes the web handoff endpoint on port `6901`; otherwise it falls back to `Xvfb :99` for browser automation without visual handoff. The browser dependency catalog explicitly installs GLib/GObject (`libglib2.0-0` on Debian bookworm, `libglib2.0-0t64` on Ubuntu noble) because CloakBrowser is a standalone Chromium binary and apt cannot infer its shared-library dependencies.
