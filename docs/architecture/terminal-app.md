...
**Implicit in-scope: well-known harmless special paths.** A small fixed set of special filesystem paths is **always** allowed regardless of the project-root boundary, via `pathscope.IsAlwaysAllowedPath` (the single source of truth shared by both `PathAllowed` and `pathAllowedNoConsume`/`OutOfScopePaths`). Today this set is just `/dev/null`: agents routinely redirect output to it (e.g. `some_cmd > /dev/null`), and prompting for it is pure friction with no security benefit since it discards data. The check runs against the normalized (absolute, symlink-resolved) path, so `/dev/./null` and other forms that resolve to `/dev/null` are covered while genuinely different device nodes (`/dev/zero`, etc.) are not. Like the state-directory allowance, this is not a consumable grant and does not depend on any user approval.
...
When the user answers, `submitApproval` (`pkg/tui/approval.go`) calls `backend.RespondSubAgentAuth(choice)`. That method is the **routing discriminator** between the two protocols: it returns `true` (and delivers the decision on the channel) **only** when a sub-agent goroutine is actually blocked, tracked by a mutex-guarded `subAgentAuthPending` flag the gate sets *before* it emits the overlay and clears when it wakes. When it returns `true` the TUI must **not** call `RunTurn` — the sub-agent resumes the still-running parent turn on its own, and the TUI just re-arms `waitEvent` on the existing channel. When it returns `false` (no sub-agent blocked) the approval belongs to the main thread and flows through `RunTurn(choice)` as above. The flag is essential because `subAgentAuthRespCh` is buffered (cap 1): without it a naive non-blocking send would *always* succeed for a **main-thread** approval, misroute it as a sub-agent response, skip `RunTurn`, and freeze the turn — the exact bug this discriminator prevents (guarded by `TestRespondSubAgentAuth_NoPendingReturnsFalse` and the TUI-side `TestSubmitApproval_MainThreadPathCallsRunTurn` / `TestSubmitApproval_SubAgentPathDoesNotCallRunTurn`). Non-code backends (platform chat) implement `RespondSubAgentAuth` as a no-op returning `false`.
...
**Plan persistence.** When the agent finalizes a plan via `announce_plan`, code mode writes it to a per-session `PLAN.md` sidecar next to the session transcript (`<sessions-dir>/<app>/<userID>/<sessionID>.PLAN.md`, resolved by `localAgentBackend.planFilePath` and set via `ChatAgent.SetPlanFilePath`). Each phase carries an optional `details` field plus an optional `files` list (each affected file marked new/modify/delete) and a `verify` command (build/test/lint), so the concrete, dependency-aware plan (blast radius + verification) is persisted, not just labels. The file is rewritten on every phase transition via a `PlanState.onChange` hook; its checkbox-per-phase format is rendered/parsed by `pkg/agent/plan_document.go`. Progress is driven two ways: delegated phases advance automatically from sub-task events, while main-thread phases are updated by the model calling the **`update_plan`** tool (`running` → `complete`/`failed`) as it works. When the model tracks the plan itself, the end-of-turn `CompleteAll` sweep is suppressed (`PlanState.IsManuallyTracked`) so the file reflects real progress instead of a bulk completion. This makes the plan survive context compaction: the `Compactor` appends a pointer to the summary instructing the model to re-read `PLAN.md`. See `docs/plan-mode-enforcement-summary.md` → "Plan persistence (PLAN.md)" and `docs/architecture/smart-compaction.md` → "Execution Plan Survival". When the session is deleted, its `PLAN.md` sidecar is removed alongside the transcript (`localAgentBackend.DeleteSession`, plus the `astonish sessions delete` / `sessions clear` CLI), so plan files never outlive their session on disk.
...
Phase → additive allow-list (`GraphPlanPhaseTools`, single source of truth; `announce_plan` / `update_plan` and the three `gplan_*` tools are always allowed):
...
When the agent finalizes via `announce_plan`, `pkg/launcher/tui_code.go` (and the matching SSE mapping in `tui_chat.go`) converts the FunctionResponse into three events: synthetic `# Execution Plan` text, a `plan_approval` item (`ApprovalKind: "plan"`, options `"Approve & implement"` / `"Request changes"` / `"Decline"`), and `done` (the turn ends). The transcript reducer promotes the text to `ItemPlan` via the `# Execution Plan` prefix (`events/transcript.go`); the approval item is omitted from the viewport and shown as an action bar under it. Do not hide the card behind a blocking overlay.

The card (`pkg/tui/plan.go`) is a structured parse of PLAN.md via `agent.ParsePlanDocument`, not a generic markdown bubble. Layout: amber-bordered frame (code mode `PlanBorder`/`PlanHeader` = 172, matching the Plan-mode composer; platform stays steel-blue 75) with numbered phases, status glyphs (`[○]`/`[●]`/`[✓]`/`[✗]`), `+`/`~`/`−` file kinds, CONTEXT / WHAT NOT TO CHANGE / VERIFY bands, and a `k/n ready|running|done` footer. Unparseable / mid-stream chunks fall back to markdown-in-a-box so history never goes blank. `_Last updated` and the checkbox legend are hidden — status is the glyphs.

Approval actions live in `handlePlanApprovalKey` (called from `handleApprovalKey` before generic y/n/esc, the same way network denials are). Mapping: Enter submits the highlighted option (cursor defaults to implement); `y`/`1` implement; `r`/`2` request changes (stay in Plan mode, composer placeholder becomes "Describe the changes to the plan…"); `n`/`esc`/`3` decline (back to Normal). Do not route plan keys through `pickYes`/`pickNo` — those would send `n`/`esc` to "Request changes". The live View path swaps the composer for `renderPlanApprovalFooter`; `renderPlanApprovalOverlay` remains only for tests.
