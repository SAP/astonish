...
- **`shell_command` filesystem scope guard (grant-blind; reserved for non-interactive callers).** `pkg/tools` exposes `tools.SetScopeRoot(root)`. When a root is set, `ShellCommand` hard-rejects any command whose operands resolve **outside** that project root (an `access denied` error, not a prompt). Path extraction + containment reuse `pkg/pathscope` — the SAME logic the agent's folder-access gate uses — so this guard and the interactive gate can never drift. **This guard is deliberately NOT wired in the interactive code-mode launcher** (`chat_factory.go` does not call `SetScopeRoot`): there, the *grant-aware* folder-access gate in `pkg/agent` is the sole enforcer — it prompts and records a grant so an approved out-of-scope path succeeds on retry. Engaging this grant-blind guard interactively would re-reject a path the user just approved (the "approved but still fails" regression). `SetScopeRoot` is reserved for genuinely non-interactive callers that cannot prompt (ReAct planner, scheduler auto-approve). The extractor (`pathscope.ExtractCommandPaths`) is a heuristic and default-deny biased: it flags absolute (`/…`), home (`~`/`~/…`), and `../…` escape tokens; it deliberately does NOT flag in-tree relative names. Its tokenizer is **quote-aware** — single/double-quoted spans are treated atomically so quoted literal data (e.g. a `git commit -m "…/…"` message) is not shredded into spurious path operands; a quoted arg is flagged only when its content is itself path-shaped. It is best-effort — shell is not fully parseable (`$(…)`, `eval`, variables, nested `sh -c "…"` can hide paths), so an empty extraction is not proof of in-scope. The guard is a **no-op when no scope root is set** (personal/CLI, Studio, interactive code mode, tests) to preserve existing behavior. Do not widen the extractor to block bare in-tree names, and do not remove the no-op-when-unset property (that would break personal mode and every existing shell test).
- **`find_files` lists immediate children by default; recursion is opt-in.** `FindFiles` is shallow by default (lists only the direct children of `search_path`, files AND dirs, like `ls`) so a "list this directory" call does not flood with the whole subtree. Set `recursive: true` — or give a pattern containing `/` or `**` — to descend. Shallow listing goes through `goListDir` (os.ReadDir + basename glob); recursive goes through ripgrep `--files` with a Go `filepath.Walk` fallback. Do not silently re-enable recursion for bare patterns: the shallow default is the fix for the "recursion floods the result" regression and the reason `shell_command` should never be used for `ls`.
