...
func TestSystemPromptBuilder_SlideGuidanceIsConditional(t *testing.T) {
	...
	for _, want := range []string{"**Slide decks:**", "skill_lookup", "ask_user", "create_deck", "write_slide", "validate_deck", "title variant", "cover image", "fixed 1920×1080 canvas", "never percentages or 0–100 coordinates", "never put Markdown markers", "shapes and theme tokens", "Do not substitute an `astonish-app`", "slideshow", "slide show", "chat text"} {
