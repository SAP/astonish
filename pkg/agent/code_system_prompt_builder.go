...
func (b *CodeSystemPromptBuilder) build(base *SystemPromptBuilder) string {
	...
	if b.PlanFilePersistence {
		...
		sb.WriteString("- `verification`: REQUIRED end-to-end command sequence that proves the user-visible outcome after all phases. In Graph-Optimized Plan mode this must include the `acceptance` sequence from `gplan_finalize`.\n")
		...
		sb.WriteString("- `outcome` per step: REQUIRED. One sentence the user could check without reading the diff.\n")
		sb.WriteString("- `verify` / `verify_kind` per step: REQUIRED. `unit` is scoped package tests for library-only work. `behavior` is a command that exercises the outcome. Running surfaces require `behavior`.\n")
